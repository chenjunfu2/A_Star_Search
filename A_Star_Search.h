...
long A_Star_Search//找到返回1，否则返回0，返回-1为内存不足
(
	...
	long lStraightLineCost = 10, //直线代价
	long lDiagonalCost = 14,//斜线代价

	CalculatH pfuncCalculatH = DefaultCalculatH, //计算当前点到目标点距离的预估函数
	SearchCallBack pfuncCurrentSearch = NULL,//获取当前走的点
	void *pSearchCallBackValue = NULL//用户自定义参数（CurrentSearch函数使用）
);